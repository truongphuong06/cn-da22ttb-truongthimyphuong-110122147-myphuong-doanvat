...
try {
    ...
    if ($search !== '') {
      ...
      if (isset($categoryPatterns[$lowerToken])) {
        ...
        $sql = "SELECT sp.* FROM san_pham sp WHERE (" . implode(' OR ', $whereParts) . ") ORDER BY sp.id DESC LIMIT $limitInt";
        ...
        $like = '%' . $search . '%';
        $stmt = $conn->prepare("SELECT sp.* FROM san_pham sp WHERE sp.ten_san_pham LIKE :s ORDER BY sp.id DESC LIMIT $limitInt");
        $stmt->bindValue(':s', $like, PDO::PARAM_STR);
        $stmt->execute();
      }
    } else {
      $stmt = $conn->query("SELECT sp.* FROM san_pham sp ORDER BY sp.id DESC LIMIT " . (int)$limit);
    }
...
<style>
/* Làm mờ sản phẩm hết hàng trên trang chủ */
    ...
    <div class="features-grid">
      <div class="feature-item">
        ...
        <div class="feature-text">
          <h3>Miễn Phí Ship</h3>
          <p>Đơn từ trên 50.000đ</p>
        </div>
      </div>
      <div class="feature-item">
        <i class="fas fa-sync-alt feature-icon"></i>
        <div class="feature-text">
          <h3>Giao trong 3 ngày</h3>
    ...
    <div class="products-grid">
      ...
      <?php else: ?>
        <?php foreach ($featured as $p):
            $so_luong = isset($p['so_luong']) ? (int)$p['so_luong'] : 0;
            $isLocked = ($so_luong <= 0) || (isset($p['trang_thai']) && $p['trang_thai'] == 0);
        ...
        <article class="product-card<?php echo $isLocked ? ' product-locked' : ''; ?>" data-id="<?php echo $productId; ?>" 
          ...
          <div class="product-image">
            ...
            <div class="product-actions">
              <button class="action-btn like-btn" data-id="<?php echo $productId; ?>" onclick="toggleLike(<?php echo $productId; ?>)" title="Yêu thích">
          ...
          <div class="product-info">
            ...
            <?php if (!empty($p['gia_giam']) && $p['gia_giam'] > 0 && $p['gia_giam'] < $p['gia']): ?>
              <div class="product-price-row">
                <span class="product-price" style="color:#e53e3e;font-weight:700;">
                  <?php echo number_format((float)($p['gia_giam']), 0, ',', '.'); ?>₫
                </span>
                <span class="product-old-price" style="text-decoration:line-through;color:#888;margin-left:8px;">
            ...
            <div class="product-meta">
              <span class="stock-badge" style="<?php echo $isLocked ? 'background:#f43f5e;color:#fff;display:none;' : ''; ?>">
                <?php echo $isLocked ? '' : 'Còn hàng'; ?>
              </span>
            </div>
            <!-- Quantity Selector -->
            <div class="quantity-selector" style="margin-top: 12px;">
              <button type="button" class="qty-btn minus" onclick="decreaseQty(this)" <?php echo $isLocked ? 'disabled style=\"opacity:0.5;cursor:not-allowed;\"' : ''; ?>>
                <i class="fas fa-minus"></i>
              </button>
              <input type="number" class="qty-input" value="1" min="1" max="99" readonly <?php echo $isLocked ? 'disabled style=\"opacity:0.5;cursor:not-allowed;\"' : ''; ?>>
              <button type="button" class="qty-btn plus" onclick="increaseQty(this)" <?php echo $isLocked ? 'disabled style=\"opacity:0.5;cursor:not-allowed;\"' : ''; ?>>
                <i class="fas fa-plus"></i>
              </button>
            </div>
            <div style="margin-top:8px;display:flex;gap:6px;flex-direction:column">
              <button type="button" class="add-cart-btn" <?php echo $isLocked ? 'disabled style=\"opacity:0.5;cursor:not-allowed;\"' : ''; ?>>
                <i class="fas fa-shopping-cart"></i> <?php echo $isLocked ? 'Hết hàng' : 'Thêm vào giỏ'; ?>
              </button>
              <button type="button" class="buy-now-btn" <?php echo $isLocked ? 'disabled style=\"opacity:0.5;cursor:not-allowed;\"' : ''; ?>>
...
<!-- Footer -->
<footer class="footer">
  <footer class="footer" style="background:#f8f9fa;border-top:2px solid #e0e0e0;margin-top:40px;box-shadow:0 2px 16px rgba(0,0,0,0.04);">
    <div class="container" style="max-width:1200px;margin:auto;padding:0 24px;">
      <div class="footer-grid" style="display:flex;flex-wrap:wrap;gap:48px;justify-content:center;align-items:flex-start;padding:48px 0 24px 0;">
        <div class="footer-col" style="flex:1;min-width:220px;text-align:center;">
          <div class="social-links" style="margin-bottom:22px;display:flex;gap:18px;justify-content:center;">
            <a href="https://facebook.com" target="_blank" class="social-btn" aria-label="Facebook" style="border:2px solid #e0e0e0;border-radius:50%;width:44px;height:44px;display:flex;align-items:center;justify-content:center;font-size:22px;color:#4267B2;background:#fff;transition:all 0.2s;box-shadow:0 2px 8px rgba(66,103,178,0.08);">
              <i class="fab fa-facebook-f"></i>
            </a>
            <a href="https://instagram.com" target="_blank" class="social-btn" aria-label="Instagram" style="border:2px solid #e0e0e0;border-radius:50%;width:44px;height:44px;display:flex;align-items:center;justify-content:center;font-size:22px;color:#E1306C;background:#fff;transition:all 0.2s;box-shadow:0 2px 8px rgba(225,48,108,0.08);">
              <i class="fab fa-instagram"></i>
            </a>
            <a href="https://twitter.com" target="_blank" class="social-btn" aria-label="Twitter" style="border:2px solid #e0e0e0;border-radius:50%;width:44px;height:44px;display:flex;align-items:center;justify-content:center;font-size:22px;color:#1DA1F2;background:#fff;transition:all 0.2s;box-shadow:0 2px 8px rgba(29,161,242,0.08);">
              <i class="fab fa-twitter"></i>
            </a>
            <a href="https://youtube.com" target="_blank" class="social-btn" aria-label="YouTube" style="border:2px solid #e0e0e0;border-radius:50%;width:44px;height:44px;display:flex;align-items:center;justify-content:center;font-size:22px;color:#FF0000;background:#fff;transition:all 0.2s;box-shadow:0 2px 8px rgba(255,0,0,0.08);">
              <i class="fab fa-youtube"></i>
            </a>
          </div>
          <div style="font-size:15px;color:#888;line-height:1.7;max-width:240px;margin:auto;">Kết nối với chúng tôi qua các kênh mạng xã hội để nhận ưu đãi và hỗ trợ nhanh nhất!</div>
        </div>

        <div class="footer-col" style="flex:1;min-width:200px;text-align:center;">
          <h3 style="font-size:1.15rem;font-weight:700;color:#222;margin-bottom:18px;letter-spacing:0.5px;">Hỗ Trợ</h3>
          <ul style="list-style:none;padding:0;margin:0;line-height:2;">
            <li><a href="#" style="color:#444;text-decoration:none;transition:color 0.2s;font-size:15px;">Hướng Dẫn Mua Hàng</a></li>
            <li><a href="#" style="color:#444;text-decoration:none;transition:color 0.2s;font-size:15px;">Phương Thức Thanh Toán</a></li>
            <li><a href="#" style="color:#444;text-decoration:none;transition:color 0.2s;font-size:15px;">Vận Chuyển & Giao Hàng</a></li>
          </ul>
          <div style="font-size:13px;color:#aaa;margin-top:10px;">Chúng tôi luôn sẵn sàng hỗ trợ bạn 24/7!</div>
        </div>

        <div class="footer-col" style="flex:1;min-width:240px;text-align:center;">
          <h3 style="font-size:1.15rem;font-weight:700;color:#222;margin-bottom:18px;letter-spacing:0.5px;">Liên Hệ</h3>
          <ul style="list-style:none;padding:0;margin:0;line-height:2;">
            <li style="display:flex;align-items:center;gap:8px;color:#444;font-size:15px;justify-content:center;margin-bottom:6px;">
              <i class="fas fa-map-marker-alt" style="color:#EF476F;font-size:17px;"></i>
              <span>123 Ngọc Nguyên, Phường 5, Vĩnh Long</span>
            </li>
            <li style="display:flex;align-items:center;gap:8px;color:#444;font-size:15px;justify-content:center;margin-bottom:6px;">
              <i class="fas fa-phone" style="color:#06D6A0;font-size:17px;"></i>
              <a href="tel:1900xxxx" style="color:#444;text-decoration:none;font-weight:600;">1900 xxxx</a>
            </li>
            <li style="display:flex;align-items:center;gap:8px;color:#444;font-size:15px;justify-content:center;">
              <i class="fas fa-envelope" style="color:#118ab2;font-size:17px;"></i>
              <a href="mailto:hello@shop.vn" style="color:#444;text-decoration:none;font-weight:600;">hello@shop.vn</a>
            </li>
          </ul>
          <div style="font-size:13px;color:#aaa;margin-top:10px;">Thời gian làm việc: 8h - 21h (T2 - CN)</div>
        </div>
      </div>

      <div class="footer-bottom" style="border-top:1px solid #e0e0e0;margin-top:18px;padding:18px 0 0 0;text-align:center;color:#888;font-size:15px;">
        <p style="margin:0;">© <?php echo date('Y'); ?> Shop. All rights reserved. Made with <i class="fas fa-heart" style="color:#EF476F;"></i> in Vietnam</p>
      </div>
    </div>
  </footer>
</footer>
...
(function(){
  ...
  document.querySelectorAll('.buy-now-btn').forEach(btn => {
    btn.addEventListener('click', function(e) {
      ...
      console.log('Product info:', {id, name, price, image, quantity});
      ...
      // Lưu lại id sản phẩm vừa mua để trang thanh toán tự động chọn
      try { localStorage.setItem('lastBuyProductId', id); } catch(e){}
